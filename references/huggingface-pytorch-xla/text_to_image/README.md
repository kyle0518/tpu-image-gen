...
Install PyTorch and PyTorch/XLA stable versions (avoid pinning a specific nightly
dev snapshot date — those builds get removed from the index and the install will
start failing; check the [pytorch/xla README](https://github.com/pytorch/xla) for
the current stable version if `2.8.0` below is no longer the latest):
```bash
